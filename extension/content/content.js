(function() {
  ...
  function findTextInDOM(root, searchText) {
    ...
    // Skip the title (first ~100 chars) — body text is more likely to match DOM text nodes
    const body = normalized.length > 150 ? normalized.substring(100) : normalized;
    const shortNeedle = body.substring(0, Math.min(120, body.length));
    ...
    while ((node = walker.nextNode())) {
      ...
      const idx = t.indexOf(shortNeedle.substring(0, 60));
      if (idx === -1) continue;
      const cmp = t.substring(idx);
      let score = 0;
      for (let i = 0; i < Math.min(shortNeedle.length, cmp.length); i++) {
        if (shortNeedle[i] === cmp[i]) score++; else break;
      }
  ...
  function getMainContentEl() {
    ...
    const selectors = [
      ...
      '.markdown-body', '.prose',
      '[role="main"]',
      'main',              // generic, try last — often wraps nav+content
    ];
  ...
  function isInsideHeading(textNode) {
    let node = textNode.parentElement;
    while (node) {
      if (HEADING_TAGS.has(node.tagName)) return true;
      node = node.parentElement;
    }
    return false;
  }

  function isInsideBlockElement(textNode) {
    let node = textNode.parentElement;
    while (node) {
  ...
  function extractAndWrapSentences(root) {
    sentences = [];
    let id = 0;
    ...
    const walker = document.createTreeWalker(
      ...
      NodeFilter.SHOW_TEXT,
      {
        acceptNode(node) {
          ...
          if (!isInsideBlockElement(node)) return NodeFilter.FILTER_REJECT;
  ...
  function getNextSiblingNode(node, target) {
    if (node.nextSibling) {
      const sib = node.nextSibling;
      if (target && sib !== target && sib.nodeType === Node.ELEMENT_NODE && sib.contains(target)) {
        return sib.firstChild;
      }
      return sib;
    }
  ...
  function mergeAdjacentSentences() {
    ...
    for (let i = 0; i < sentences.length - 1; i++) {
      ...
      // Find next non-removed sentence (skip indices already removed by previous merges)
      let j = i + 1;
      ...
      if (!current.element.parentNode || !next.element.parentNode) continue;

      if (/[.!?…]$/.test(current.text)) continue;

      let node = current.element;
      let areConsecutive = false;

      while ((node = getNextSiblingNode(node, next.element))) {
        while (node.nodeType === Node.ELEMENT_NODE &&
               !node.classList.contains('ra-sentence') &&
               node.contains && node.contains(next.element) &&
               node.firstChild) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE &&
          ...
          if (!node.contains || !node.contains(next.element)) break;
        }
      }
  ...
  function stopReading(notify = true) {
    ...
    updateProgress();
    if (notify) {
      chrome.runtime.sendMessage({ action: 'stateChanged', playing: false, paused: false }).catch(() => {});
    }
  }
  ...
  function pauseReading() {
    ...
    updateControlBarPlayButton();
    chrome.runtime.sendMessage({ action: 'stateChanged', playing: false, paused: true }).catch(() => {});
  }
  ...
  function resumeReading() {
    ...
    updateControlBarPlayButton();
    chrome.runtime.sendMessage({ action: 'stateChanged', playing: true, paused: false }).catch(() => {});
  }
  ...
  function prependTitle() {
    ...
    if (firstText.includes(titleStart)) return;

    // Create virtual sentence for the title (hidden, audio only)
    const titleSpan = document.createElement('span');
    titleSpan.className = 'ra-sentence ra-temp-sentence';
  ...
  function startReading() {
    ...
    // Prepend article title if not already the first sentence
    prependTitle();
  ...
  function showSelectionButton(x, y) {
    ...
    selectionButtonEl.addEventListener('click', () => {
      ...
      if (sel && sel.toString().trim()) {
        readSelectedText(sel.toString().trim());
      }
      hideSelectionButton();
    });
  ...
  async function readSelectedText(text) {
    ...
    // For extremely large selections, fall back to text-only to avoid DOM churn
    if (text.length > 20000) {
      const container = document.createElement('div');
      container.id = 'ra-selection-container';
      container.style.cssText = 'position:fixed;top:-9999px;left:-9999px;';
      document.body.appendChild(container);

      const parts = text.match(SENTENCE_REGEX) || [text];
      sentences = parts
        .filter((t) => t.trim().length > 0)
        .map((t, i) => {
          const span = document.createElement('span');
          span.className = 'ra-sentence ra-temp-sentence';
          span.dataset.raId = String(i);
          span.textContent = t;
          container.appendChild(span);
          return { id: i, text: t.trim(), element: span, _temp: true };
        });

      selection.removeAllRanges();
    } else {
      // Extract + wrap in place (images pass through untouched)
